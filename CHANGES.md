...
## 2026-08-24 (later) — Two user-reported bugs fixed

### BUG 1 — Create Invoice screen went blank (white) for some tenants
**Root cause:** a tenant record missing the `currency` field (`null`) caused the builder's
currency formatter / render to throw, blanking the whole Create-Invoice view. (A separate
"suspended" tenant correctly shows a "workspace suspended" message — that is by design, not the bug.)

**Fix:**
- `src/server/db.ts` now **normalizes on load**: every tenant gets `currency:'RM'` and
  `paperSize:'A4 (Standard)'`, every invoice gets `currency:'RM'` if missing. Already-persisted
  stores are patched in place on startup (idempotent).
- `InvoiceBuilderView` currency state falls back to `tenant.currency || 'RM'`.
- Verified: a tenant with `currency:null` now renders Create Invoice fully (was blank before).

### BUG 2 — Downloaded PDF looked different from the live preview
**Root cause:** the download (`/i/:number`) used a completely separate hand-written HTML template
from the on-screen builder preview — two divergent designs.

**Fix (single source of truth):**
- New `src/components/InvoiceDocument.tsx` — a presentational component (inline-styled, defensive
  against missing fields) that renders the exact invoice layout. Inline styles mean it renders
  identically in the browser AND server-side without depending on Tailwind CDN.
- `InvoiceBuilderView` live preview now renders `<InvoiceDocument/>` (replaces the old inline JSX).
- Server routes `GET /i/:number` and `GET /i/example` now SSR `<InvoiceDocument/>` via
  `react-dom/server` `renderToStaticMarkup` (replacing the old `renderInvoiceHtml`).
- Result: the downloaded PDF / `/i/:number` page is the SAME component as the preview — same fonts,
  colors, watermark, totals, remittance. Builder "Print / PDF" and list "Download" both match the
  on-screen preview. Verified visually (screenshot of `/i/INV-2023-1042` shows the branded design).
